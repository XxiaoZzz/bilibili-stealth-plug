...
7. 鼠标移回小窗：窗口透明度恢复到你设置的可见态透明度，视频继续播放。
8. 鼠标移到 `亮度` 或 `透明` 按钮上，会弹出垂直滑杆，拖动即可实时调节。

## 说明

- 这版是真正控制原生窗口透明度，不依赖 Chrome PiP 外壳。鼠标移出时会自动暂停，移回时只恢复由隐身模式自动暂停的视频，并恢复到你设置的可见态透明度。
...
- `亮度`：鼠标悬停时弹出垂直滑杆，实时调节小窗视频亮度，范围 `50% ~ 150%`。
- `透明`：鼠标悬停时弹出垂直滑杆，实时调节小窗可见状态透明度，范围 `20% ~ 100%`。鼠标移出后仍会完全隐藏，鼠标移回后恢复到你设置的透明度。
