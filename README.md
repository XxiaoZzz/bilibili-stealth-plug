...
- 鼠标移入小窗：窗口恢复到你设置的“可见状态透明度”，并继续播放由隐身模式自动暂停的视频。
- 支持播放/暂停、当前时间、可拖动进度条、总时长。
- 支持倍速选择按钮：可直接选择 `0.75x / 1x / 1.25x / 1.5x / 2x / 3x / 4x / 5x`。
- 支持亮度和透明度悬浮滑杆：鼠标移到 `亮度` / `透明` 按钮上，会弹出垂直滑杆并实时调节。
- 支持小窗进度回写：关闭透明小窗后，原网页视频进度同步到小窗最新进度；再次点击“透明”会从该进度继续。
...
flowchart LR
  ...
  F --> H["鼠标移入: setOpacity(可见透明度基线) + play()"]
```
...
7. 鼠标移回小窗：窗口恢复到你当前设置的可见态透明度，视频继续播放。
8. 鼠标移入小窗底部：显示播放/暂停、进度条、倍速按钮、亮度按钮、透明按钮、弹幕开关。
9. 鼠标移到 `亮度` 或 `透明` 按钮上，会弹出垂直滑杆，拖动即可实时调节。
10. 关闭透明小窗后，原 B 站网页的视频进度会同步到小窗最新播放进度；再次点击“透明”会从同步后的进度继续。
...
6. 鼠标移入小窗底部可以拖动进度条，也可以点击倍速按钮展开菜单，或把鼠标移到 `亮度` / `透明` 按钮上弹出垂直滑杆实时调节。
7. 关闭透明小窗后，原 B 站网页的视频进度会同步到小窗最新播放进度；再次点击“透明”会从同步后的进度继续。
...
控制条只在鼠标移入小窗时显示。把鼠标移动到小窗底部即可看到播放按钮、进度条、倍速按钮、亮度按钮、透明按钮和弹幕开关。亮度/透明按钮支持悬停后弹出垂直滑杆实时调节。
